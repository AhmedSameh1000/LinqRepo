	...
	internal class Program
	{
		...
		private static void DemoExecutionOrder()
		{
			...
			var query = numbers
				.Where(x =>
				{
					Console.WriteLine("start firt Where");
					Console.WriteLine($"Where({x} > 5) => {x > 5}");
					return x > 5;
				})
				.Select(x =>
				{
					Console.WriteLine("start firt Select");
					Console.WriteLine($"\tSelect({x} X {x}) => {x * x}");
					return x * x;
				})
				.Where(x =>
				{
					Console.WriteLine("start Second Where");
		...
		private static void RunQuery()
		{
			...
			{
				return x.IsRed;
			})   // {             9 Diamonds, 4 Hearts,            3 Hearts, 6 Hearts }
			.Skip(1)               // {                       , 4 Hearts,            3 Hearts, 6 Hearts }
			.OrderBy(x =>
			{
				return x.Value;
			}) // {                       , 3 Hearts,            4 Hearts, 6 Hearts }
			.Take(2)
